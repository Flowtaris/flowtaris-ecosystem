...
import { getAssessmentConfigs } from '@/lib/supabase'
...
export default function AssessmentConfigPage() {
  const [configs, setConfigs] = useState<AssessmentConfigFormData[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // Fetch assessment configs on load
  useEffect(() => {
    const fetchAssessmentConfigs = async () => {
      try {
        setLoading(true)
        const data = await getAssessmentConfigs()
        // Convert the data to our form format, ensuring JSON fields are strings
        const formData: AssessmentConfigFormData[] = data.map((config: any) => ({
          id: config.id,
          questions: JSON.stringify(config.questions, null, 2),
          recommendation_rules: JSON.stringify(config.recommendation_rules, null, 2),
          capability_mapping: JSON.stringify(config.capability_mapping, null, 2),
        }))
        setConfigs(formData)
      } catch (err: any) {
        setError('Failed to load assessment configs')
        console.error(err)
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/admin/admin/assessment-config/new" className="bg-brand-cyan-600 hover:bg-brand-cyan-700 text-white font-bold py-2 px-4 rounded">
      ...
      {configs.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-500 dark:text-gray-400">No assessment configurations found. Create your first assessment config.</p>
        </div>
      ) : (
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-200">
                    Assessment Config {config.id.substring(0, 8)}
                  </h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Questions: {JSON.parse(config.questions).length} items
                  </p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Capability Mappings: {Object.keys(JSON.parse(config.capability_mapping)).length} capabilities
                  </p>
                </div>
                <div className="space-x-2">
                  <Link href={`/assessment-config/${config.id}/edit`} className="text-sm text-brand-cyan-600 hover:text-brand-cyan-700">
                    Edit
                  </Link>
                </div>
              </div>
